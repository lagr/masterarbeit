...
\label{sub:workflow_activity_images}
  ...
  \texttt{FileHelper}, \texttt{Configuration}, and \texttt{Validator} have the same functionalities as their counterparts in the workflow image. The image should be able to start a subworkflow or a third-party container, or create a worklist item and wait for its completion. The classes \texttt{SubworkflowInvocation}, \texttt{ContainerInvocation}, and \texttt{WorklistClient} encompass these functionalities respectively. The decision as to which class is invoked for the actual functionality is made by the \texttt{Instance} class.
