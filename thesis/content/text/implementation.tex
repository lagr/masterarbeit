...
\label{sec:exemplary_deployment}
  ...
  As an exemplary use case, a workflow is modeled and executed which consists of two sub-workflow activities, whereas one sub-workflow contains a third-party container activity and the other a manual activity. When the installation of the prototype is finished, the developer gateway is visited with a browser at the network address of the \texttt{development-machine} on port 3000 (Figure~\ref{fig:dashboard}). First, a role is added, then a user (Figure~\ref{fig:dev_role}, Figure~\ref{fig:dev_user}).
